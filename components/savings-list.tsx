...
import { createSavingsColumns } from '@/components/transactions/savings-columns';
import {
  Table,
  TableBody,
  TableCell,
...
export function SavingsList({ refreshKey, monthFilter, onTransactionCreated }: SavingsListProps) {
  const [savings, setSavings] = useState<PaginatedTransactions | null>(null);
  ...
  useEffect(() => {
    async function fetchSavingsAndCategories() {
      try {
        setIsLoading(true);
        setError(null);
        const [savingsData, categoriesData] = await Promise.all([
          getSavings({
            page: 1,
            limit: 20,
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to load savings and categories';
        setError(errorMessage);
      } finally {
        setIsLoading(false);
      }
    }
    void fetchSavingsAndCategories();
  ...
  const handleEditClick = useCallback(
    (transactionId: string): void => {
      ...
      const transactionToEdit = savings.data.find(
  ...
  const handleSubmitEdit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setSavings((currentSavings) => {
        ...
        const updatedData = currentSavings.data.map((transaction) =>
      ...
      toast({
        ...
        description: `Saving "${updatedTransaction.label}" has been updated successfully.`,
      });
      handleCancelEdit();
    } catch (updateError) {
      const errorMessage =
        updateError instanceof Error ? updateError.message : 'Failed to update saving';
      toast({
        title: 'Error',
  ...
  const handleDeleteClick = useCallback(
    (transactionId: string): void => {
      const transactionToDelete = savings?.data.find(
        (transaction) => transaction.id === transactionId,
      );
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      setSavings((currentSavings) => {
        ...
        const filteredData = currentSavings.data.filter(
      ...
      toast({
        ...
        description: deleteResponse.message || 'The saving has been deleted successfully.',
      });
      setDeletingTransactionId(null);
      setDeleteTransactionLabel('');
    } catch (deleteError) {
      const errorMessage =
        deleteError instanceof Error ? deleteError.message : 'Failed to delete saving';
      toast({
        title: 'Error',
        description: errorMessage,
        variant: 'destructive',
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      {isLoading && <div className="text-center text-muted-foreground">Loading savings...</div>}
      ...
      {!isLoading && !error && savings && (
        <>
          {savings.data.length === 0 ? (
            <div className="rounded-md border p-4 text-center text-muted-foreground">
              No savings found for the selected month.
            </div>
          ) : (
