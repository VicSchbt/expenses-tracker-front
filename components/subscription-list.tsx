...
export function SubscriptionList({ refreshKey, monthFilter }: SubscriptionListProps) {
  ...
  useEffect(() => {
    async function fetchSubscriptionsAndCategories() {
      try {
        setIsLoading(true);
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Failed to load subscriptions and categories';
  ...
  const handleSubmitEdit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setSubscriptions((currentSubscriptions) => {
        ...
        const updatedData = currentSubscriptions.data.map((transaction) =>
      ...
      toast({
        ...
        description: `Subscription "${updatedTransaction.label}" has been updated successfully.`,
      });
      handleCancelEdit();
    } catch (updateError) {
      const errorMessage =
  ...
  const handleDeleteClick = useCallback(
    (transactionId: string): void => {
      const transactionToDelete = subscriptions?.data.find(
        (transaction) => transaction.id === transactionId,
      );
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    try {
      ...
      setSubscriptions((currentSubscriptions) => {
        ...
        const filteredData = currentSubscriptions.data.filter(
      ...
      toast({
        ...
        description: deleteResponse.message || 'The subscription has been deleted successfully.',
      });
